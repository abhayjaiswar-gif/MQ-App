  ...
  <v-card class="bg-white rounded-[2.5rem] shadow-sm border border-slate-100 overflow-hidden" variant="text">
    <v-card-text class="pa-6">
      <div class="flex items-center justify-between mb-6">
        ...
        <!-- All users can add milestones for their assigned school -->
      ...
      <div class="bg-white rounded-3xl p-4 mb-6 border border-slate-50 shadow-sm">
        <div class="flex items-center justify-between mb-4 px-2">
          <span class="text-[11px] font-black text-slate-900 uppercase tracking-tight">{{ currentMonthName }} {{
            currentYear }}</span>
          <div class="flex gap-1">
            <v-btn icon density="compact" variant="text" size="x-small" @click="prevMonth"><span
                class="material-symbols-outlined text-xs">chevron_left</span></v-btn>
            <v-btn icon density="compact" variant="text" size="x-small" @click="nextMonth"><span
                class="material-symbols-outlined text-xs">chevron_right</span></v-btn>
          </div>
        </div>
        <div class="grid grid-cols-7 gap-1 mb-2 text-center">
          <div v-for="day in ['S', 'M', 'T', 'W', 'T', 'F', 'S']" :key="day"
            class="text-[8px] font-black text-slate-300">{{
              day }}</div>
        </div>
        <div class="grid grid-cols-7 gap-1">
          <div v-for="(day, idx) in calendarDays" :key="idx"
            ...
            <div v-if="day.events.length" class="flex gap-0.5 mt-0.5">
              <div v-for="e in day.events.slice(0, 2)" :key="e.id" class="w-1 h-1 rounded-full"
                :class="getDotColor(e.category)"></div>
            </div>
            <div v-if="day.isToday"
              class="absolute -top-0.5 -right-0.5 w-1.5 h-1.5 bg-primary rounded-full border border-white"></div>
          </div>
        </div>
      </div>
      ...
      <div class="space-y-4">
        ...
        <div v-if="upcomingEvents.length"
          class="relative before:absolute before:inset-y-0 before:left-[19px] before:w-px before:bg-slate-100 space-y-4 pl-1 max-h-[260px] overflow-y-auto custom-scroll pr-2 pb-2">
          <div v-for="(event, i) in upcomingEvents" :key="event.id" class="relative pl-12">
            <!-- Timeline Dot -->
            <div
              class="absolute left-[-2px] top-1/2 -translate-y-1/2 w-[42px] h-[42px] rounded-xl flex flex-col items-center justify-center shadow-sm border-[3px] border-white z-10"
              :class="getCategoryTag(event.category)">
            ...
            <!-- Content Card -->
            <div
              class="p-3 bg-white border border-slate-50 rounded-2xl shadow-sm hover:-translate-y-1 transition-transform cursor-pointer">
              <div class="flex items-center justify-between gap-2 mb-1">
                <h5 class="text-[10px] font-black text-slate-800 truncate">{{ event.title }}</h5>
                <div
                  class="flex items-center gap-1 shrink-0 rounded-full bg-slate-50 px-2 py-0.5 border border-slate-100">
                  <span class="w-1.5 h-1.5 rounded-full" :class="getDotColor(event.category)"></span>
                  <span class="text-[8px] font-black text-slate-500 uppercase">{{ getCountdownFormat(event.event_date)
                    }}</span>
                </div>
              </div>
    ...
    <v-dialog v-model="showAddModal" max-width="420">
      <v-card class="rounded-[2.5rem] pa-8 bg-white border border-slate-100">
        <v-card-title class="px-0 pt-0 text-h5 font-black text-slate-900 pb-4 flex justify-between items-center">
          Add Milestone
        ...
        <v-form @submit.prevent="saveEvent" class="space-y-4">
          ...
          <!-- School Selector: shown when user has 2+ schools OR is admin -->
          <template v-if="assignedSchools.length > 1 || isAdmin">
            <v-select
              v-model="form.school_id"
              :items="assignedSchools"
              item-title="name"
              item-value="id"
              label="Select School"
              variant="outlined"
              density="comfortable"
              class="rounded-xl custom-field"
              prepend-inner-icon="mdi-school-outline"
              :rules="[v => !!v || 'Please select a school']"
            ></v-select>
          </template>

          <!-- Single school: show as read-only badge -->
          <template v-else-if="assignedSchools.length === 1">
            <div class="px-3 py-2.5 rounded-xl bg-slate-50 border border-slate-100 flex items-center gap-2">
              <span class="material-symbols-outlined text-sm text-primary">school</span>
              <span class="text-[11px] font-black text-slate-600 uppercase tracking-widest">{{ assignedSchools[0].name }}</span>
            </div>
          </template>

          <!-- No school -->
          <template v-else>
            <div class="px-3 py-2.5 rounded-xl bg-red-50 border border-red-100 flex items-center gap-2">
              <span class="material-symbols-outlined text-sm text-red-400">warning</span>
              <span class="text-[11px] font-black text-red-400 uppercase tracking-widest">No School Assigned</span>
            </div>
          </template>

          <v-select v-model="form.category" :items="['Sports', 'Academic', 'National']" label="Event Type" variant="outlined"
            density="comfortable" class="rounded-xl custom-field"></v-select>
          <v-text-field v-model="form.title" label="What's happening?" variant="outlined" density="comfortable"
            class="rounded-xl custom-field"></v-text-field>
          <v-text-field v-model="form.event_date" type="date" label="When?" variant="outlined" density="comfortable"
            class="rounded-xl custom-field"></v-text-field>
          <v-text-field v-model="form.description" label="Description (optional)" variant="outlined" density="comfortable"
            class="rounded-xl custom-field"></v-text-field>
          <v-btn
            type="submit" block color="primary"
            :disabled="!form.title || !form.event_date || ((assignedSchools.length > 1 || isAdmin) && !form.school_id)"
            class="rounded-xl h-[48px] font-black text-[10px] uppercase tracking-widest mt-2 shadow-lg"
          >Set Milestone</v-btn>
        </v-form>
      </v-card>
    </v-dialog>
...
const form = ref<any>({ category: 'Sports', title: '', event_date: '', description: '', school_id: null, is_featured: 0 });

// Fetch schools this user is assigned to (or all schools for admin)
const fetchAssignedSchools = async () => {
  try {
    const url = isAdmin.value
      ? '/api/schools'
      : `/api/schools?user_id=${userId}`;
    ...
    if (data.success) {
      ...
      if (assignedSchools.value.length === 1) {
        form.value.school_id = assignedSchools.value[0].id;
      }
    }
  } catch (err) { console.error('fetchAssignedSchools error:', err); }
};
...
const saveEvent = async () => {
  try {
    ...
    if (data.success) {
      ...
      // Reset form but keep auto-selected school if only 1
      const autoSchool = assignedSchools.value.length === 1 ? assignedSchools.value[0].id : null;
      form.value = { category: 'Sports', title: '', event_date: '', description: '', school_id: autoSchool, is_featured: 0 };
      fetchAll();
    }
  } catch (err) { console.error(err); }
};
