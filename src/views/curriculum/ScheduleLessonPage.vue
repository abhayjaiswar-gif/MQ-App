      ...
      <div class="col-span-12 lg:col-span-8 space-y-8">
        <section class="bg-surface-container-lowest p-8 rounded-2xl shadow-sm border border-slate-100">
          ...
          <div class="space-y-10">
            <!-- Number of Chapters -->
            <div>
              <label class="block text-sm font-semibold text-on-surface-variant mb-4 font-body">Number of Chapters/Sessions</label>
              <div class="flex items-center max-w-xs bg-surface-container rounded-xl p-1 overflow-hidden">
                <button @click="form.chapters > 1 ? form.chapters-- : null" class="p-3 hover:bg-white rounded-lg transition-colors text-primary flex items-center justify-center">
                  <span class="material-symbols-outlined">remove</span>
                </button>
                <input v-model="form.chapters" class="flex-1 bg-transparent border-none text-center font-bold text-lg focus:ring-0 outline-none w-16" type="number"/>
                <button @click="form.chapters++" class="p-3 hover:bg-white rounded-lg transition-colors text-primary flex items-center justify-center">
            ...
            <!-- Weekly Recurrence -->
            <div>
              <label class="block text-sm font-semibold text-on-surface-variant mb-4 font-body">Weekly Recurrence</label>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div>
                ...
                <div class="relative">
                  ...
                  <input v-model="form.startTime" class="w-full pl-12 pr-4 py-3 bg-surface-container border-none rounded-xl font-medium focus:ring-2 focus:ring-primary/20 outline-none" type="time"/>
                </div>
              </div>
              <div>
                ...
                <div class="relative">
                  <span class="material-symbols-outlined absolute left-4 top-1/2 -translate-y-1/2 text-outline">schedule</span>
                  <input v-model="form.endTime" class="w-full pl-12 pr-4 py-3 bg-surface-container border-none rounded-xl font-medium focus:ring-2 focus:ring-primary/20 outline-none" type="time"/>
                </div>
              </div>
            </div>
          </div>
        ...
        <div class="flex items-center justify-end space-x-4 pt-4">
          <button class="px-8 py-4 rounded-xl font-bold text-on-surface-variant hover:bg-surface-container transition-colors font-body">Cancel</button>
          <button 
            @click="handleSubmit" 
            :disabled="submitting"
        ...
        <Transition name="fade">
          <div v-if="successMsg" class="bg-green-100 border border-green-200 text-green-800 p-4 rounded-xl flex items-center justify-between shadow-sm">
        ...
        <section class="bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden mt-12">
          <div class="p-6 border-b border-slate-50 flex items-center justify-between bg-[#f9f9f9]/50">
            ...
            <button @click="fetchSchedules" class="text-primary hover:bg-primary/5 p-2 rounded-lg transition-colors flex items-center gap-1 text-xs font-bold uppercase tracking-widest">
            ...
            <table class="w-full text-left">
              <thead>
                <tr class="bg-slate-50">
                  ...
                  <th class="px-6 py-4 text-[10px] font-black uppercase tracking-widest text-slate-400">Scheduled For</th>
              ...
              <tbody class="divide-y divide-slate-100">
                ...
                <tr v-else-if="schedules.length === 0">
                  <td colspan="5" class="py-12 text-center text-slate-400 italic text-sm">No assignments scheduled yet.</td>
                </tr>
                <tr v-for="item in schedules" :key="item.id" class="hover:bg-slate-50/50 transition-colors group">
                  <td class="px-6 py-4">
                    <span class="text-[10px] font-black bg-slate-100 px-2 py-1 rounded text-slate-500">#{{ item.id }}</span>
                  </td>
                  <td class="px-6 py-4">
        ...
        <div class="bg-secondary-container/30 border border-secondary-container/50 p-8 rounded-3xl sticky top-28">
            ...
            <div class="flex items-start space-x-4">
              ...
              <div>
                ...
                <p class="text-lg font-bold text-on-surface leading-snug">
                  {{ form.recurrence.length > 0 ? form.recurrence.join(' & ') : 'Please select days' }}, {{ formatTime(form.startTime) }} - {{ formatTime(form.endTime) }}
                </p>
              </div>
            </div>
          </div>
          ...
          <div class="mt-10 p-5 bg-white/60 rounded-2xl border border-white/40 shadow-sm backdrop-blur-sm">
            ...
            <p class="text-[11px] text-on-surface-variant leading-relaxed">
              Generating the schedule will automatically allocate training pitches and assign the primary instructor for Grade 8. You can manually override specific dates after generation.
            </p>
          </div>
          <div class="mt-8 overflow-hidden rounded-2xl shadow-lg shadow-black/5 ring-1 ring-white/20">
            <img alt="Football training pitch" class="w-full h-32 object-cover hover:scale-105 transition-transform duration-700" src="https://lh3.googleusercontent.com/aida-public/AB6AXuCAIozymi0KfM4Q7kskP15rbc7TawU7JjxD3FWUu4PW8YgPmnlhKAjpCcKVE29GcfR_VMHbMM6d1e-sStwcNm_34sdxy0ueFm8nSRAobgI-t0CfIV2cwNuTbjvIXSUrqvYQAIAXLaPvsc5QVK0vAQt4auQrOnSwphnrPy544BwIET-wGIQw-bdRLwFp9XBhXNOdtvs6Q0zVsswFgEngxlxCqp33NZFyV-9rx5x-EmKXDqziR_WdxlbEkZqenGfu1B0aEs6f3RZvuzpv"/>
          </div>
        </div>
      </div>
    </div>
