...
<!-- Edit Parameter Modal -->
    <div v-if="showEditModal" 
        ...
        <div class="bg-white w-full max-w-4xl rounded-3xl shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200" @click.stop>
            <!-- Modal Header -->
            <div class="px-8 py-6 bg-slate-50 border-b border-slate-100 flex items-center justify-between">
                <div>
                    ...
                    <p v-if="editingParam.id" class="text-xs font-bold text-slate-400 uppercase tracking-widest mt-1">ID: {{ editingParam.id }} • {{ editingParam.parameter }}</p>
                    <p v-else class="text-xs font-bold text-slate-400 uppercase tracking-widest mt-1">New System Constant</p>
                </div>
                <button @click="closeEditModal" class="p-2 hover:bg-slate-200/50 rounded-xl transition-colors text-slate-400">
            ...
            <div class="p-8 max-h-[70vh] overflow-y-auto custom-scrollbar font-inter">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div class="space-y-6">
                        <div class="space-y-2">
                            <label class="text-[10px] font-bold text-slate-400 uppercase tracking-widest font-manrope ml-1">Title*</label>
                            <input v-model="editingParam.title" @input="!editingParam.id && (editingParam.parameter = editingParam.title.toLowerCase().replace(/\s+/g, '_'))" type="text" class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all text-sm font-semibold text-slate-700 outline-none" placeholder="e.g., Speed">
                        </div>

                        <div v-if="!editingParam.id" class="space-y-2">
                            <label class="text-[10px] font-bold text-slate-400 uppercase tracking-widest font-manrope ml-1">System Key (Parameter Name)*</label>
                            <input v-model="editingParam.parameter" type="text" class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all text-sm font-semibold text-slate-700 outline-none" placeholder="e.g., vertical_jump">
                        </div>

                        <div class="space-y-2">
                            <label class="text-[10px] font-bold text-slate-400 uppercase tracking-widest font-manrope ml-1">Description</label>
                            <textarea v-model="editingParam.description" rows="4" class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all text-sm font-medium text-slate-600 outline-none resize-none" placeholder="Detail the parameter's purpose..."></textarea>
                        </div>

                        <div class="space-y-2">
                            <label class="text-[10px] font-bold text-slate-400 uppercase tracking-widest font-manrope ml-1">Test Display Name</label>
                            <input v-model="editingParam.test_display" type="text" class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all text-sm font-semibold text-slate-700 outline-none" placeholder="e.g., 50-Yards Dash">
                    ...
                    <div class="space-y-6">
                        <div class="grid grid-cols-2 gap-4">
                            <div class="space-y-2">
                                <label class="text-[10px] font-bold text-slate-400 uppercase tracking-widest font-manrope ml-1">Data Type</label>
                                <select v-model="editingParam.ctype" class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all text-sm font-bold text-slate-700 outline-none cursor-pointer">
                            ...
                            <div class="space-y-2">
                                <label class="text-[10px] font-bold text-slate-400 uppercase tracking-widest font-manrope ml-1">Measured In</label>
                                <input v-model="editingParam.clabel" type="text" class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all text-sm font-semibold text-slate-700 outline-none" placeholder="e.g., Seconds / Minutes">
                            </div>
                        </div>

                        <div class="space-y-2">
                            <label class="text-[10px] font-bold text-slate-400 uppercase tracking-widest font-manrope ml-1">YouTube Video Link</label>
                            <input v-model="editingParam.video_link" type="text" class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all text-sm font-semibold text-slate-700 outline-none" placeholder="https://youtube.com/...">
                        </div>

                        <div class="grid grid-cols-2 gap-4">
                            <div class="space-y-2">
                                <label class="text-[10px] font-bold text-slate-400 uppercase tracking-widest font-manrope ml-1">Heading Above Chart</label>
                                <input v-model="editingParam.chart_heading" type="text" class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all text-sm font-semibold text-slate-700 outline-none" placeholder="Chart Title">
                            </div>
                            <div class="space-y-2">
                                <label class="text-[10px] font-bold text-slate-400 uppercase tracking-widest font-manrope ml-1">Chart Format</label>
                                <select v-model="editingParam.chart_format" class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all text-sm font-bold text-slate-700 outline-none cursor-pointer">
                        ...
                        <div class="p-4 bg-slate-50 rounded-2xl flex items-center justify-between border border-slate-100">
                            <div>
                                ...
                                <p class="text-[10px] font-bold text-slate-400 uppercase mt-0.5 tracking-wider">{{ editingParam.is_active == 1 ? 'Parameter Active' : 'Parameter Inactive' }}</p>
                            </div>
                            <button @click="editingParam.is_active = (editingParam.is_active == 1 ? 0 : 1)" 
                                ...
                                <div :class="['w-4 h-4 bg-white rounded-full transition-all duration-300 shadow-sm', editingParam.is_active == 1 ? 'translate-x-6' : 'translate-x-0']"></div>
                            </button>
                        </div>
                    </div>
                </div>
            ...
            <div class="px-8 py-5 bg-slate-50 border-t border-slate-100 flex items-center justify-end gap-3">
                <button @click="closeEditModal" class="px-6 py-2.5 text-sm font-bold text-slate-500 hover:text-slate-700 transition-colors uppercase tracking-widest font-manrope">Cancel</button>
    ...
    <div v-if="showGradingModal" 
        ...
        <div class="bg-white w-full max-w-4xl rounded-3xl shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200" @click.stop>
            ...
            <div class="px-8 py-6 bg-slate-50 border-b border-slate-100 flex items-center justify-between">
                <div>
                    <h2 class="text-xl font-extrabold text-slate-800 font-manrope tracking-tight">Configure Performance Levels</h2>
                    <p class="text-xs font-bold text-slate-400 uppercase tracking-widest mt-1">Define thresholds and behavioral descriptors</p>
                </div>
                <button @click="showGradingModal = false" class="p-2 hover:bg-slate-200/50 rounded-xl transition-colors text-slate-400">
                    <span class="material-symbols-outlined">close</span>
                </button>
            </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div v-for="tier in gradingTiers" :key="tier.id" 
                        ...
                        <div class="flex items-center gap-3 mb-5">
                            <div :class="['w-10 h-10 rounded-xl flex items-center justify-center', tier.color === 'green' ? 'bg-emerald-50 text-emerald-600' : tier.color === 'blue' ? 'bg-blue-50 text-primary' : tier.color === 'amber' ? 'bg-amber-50 text-amber-600' : 'bg-rose-50 text-rose-600']">
                        ...
                        <div class="space-y-5">
                            <div class="space-y-2">
                                <label class="text-[10px] font-bold text-slate-400 uppercase tracking-widest font-manrope ml-1">Range Threshold</label>
                                <div class="flex items-center gap-3">
                                    <template v-if="tier.name === 'Needs Improvement'">
                                        <span class="text-sm font-bold text-slate-400 uppercase tracking-widest font-manrope">Below</span>
                                        <div class="flex-1 flex items-center gap-2">
                                            <input v-model="tier.max_percent" type="number" class="w-full px-4 py-2 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all text-sm font-bold text-slate-700 outline-none">
                                            <span class="text-sm font-bold text-slate-400">%</span>
                                        </div>
                                    </template>
                                    <template v-else>
                                        <div class="flex-1 flex items-center gap-2">
                                            <input v-model="tier.min_percent" type="number" class="w-full px-4 py-2 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all text-sm font-bold text-slate-700 outline-none">
                                            <span class="text-sm font-bold text-slate-400">%</span>
                                        </div>
                            ...
                            <div class="space-y-2">
                                <label class="text-[10px] font-bold text-slate-400 uppercase tracking-widest font-manrope ml-1">Behavioral Descriptor</label>
                                <textarea v-model="tier.descriptor" rows="3" class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all text-xs font-semibold text-slate-600 outline-none resize-none leading-relaxed" placeholder="Describe this level..."></textarea>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <!-- Modal Footer -->
            <div class="px-8 py-5 bg-slate-50 border-t border-slate-100 flex items-center justify-end gap-3">
                <button @click="showGradingModal = false" class="px-6 py-2.5 text-sm font-bold text-slate-500 hover:text-slate-700 transition-colors uppercase tracking-widest font-manrope font-bold">Cancel</button>
