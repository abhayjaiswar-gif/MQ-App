...
const fetchSchools = async () => {
  try {
    const response = await fetch('http://localhost:3000/api/schools?hasGallery=true');
    ...
    if (data.success) {
      schools.value = data.schools.map((school: any) => ({
        ...
        thumbnail: school.school_image 
          ? `http://localhost:3000/uploads/${school.school_image}` 
          : (school.logo_image ? `http://localhost:3000/uploads/${school.logo_image}` : getRandomPlaceholder(school.id))
      }));
    }
  } catch (error) {
...
const fetchAvailableSchools = async () => {
  try {
    const response = await fetch('http://localhost:3000/api/schools?hasGallery=false');
    const data = await response.json();
    if (data.success) {
...
<template>
  <div class="gallery-root bg-white text-on-surface min-h-screen relative overflow-x-hidden">
      ...
      <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md border border-slate-100 overflow-hidden">
        ...
        <div class="p-8 space-y-6">
          <div class="space-y-2">
            ...
            <div class="relative">
              <select v-model="selectedSchoolId" class="w-full bg-slate-50 border border-slate-200 rounded-xl py-3 px-4 text-sm focus:ring-2 focus:ring-primary/20 outline-none appearance-none transition-all">
          ...
          <p class="text-xs text-on-surface-variant leading-relaxed">
            Select a school to initialize its photo archive. You'll be redirected to the gallery detail page to upload the first set of photos.
          </p>
        </div>
        <div class="px-8 py-6 bg-slate-50/50 flex items-center justify-end gap-3">
          <button @click="showAddModal = false" class="px-6 py-2.5 text-sm font-bold text-slate-500 hover:text-slate-700">Cancel</button>
          <button 
    ...
    <main class="gallery-content bg-white">
      <div class="p-8 max-w-[1400px] mx-auto">
        ...
        <div class="flex flex-col md:flex-row md:items-end justify-between mb-10 gap-4">
          ...
          <button @click="openAddModal" class="flex items-center gap-2 bg-gradient-to-v from-primary to-primary-container text-white px-5 py-3 rounded-xl font-bold text-sm shadow-lg shadow-primary/20 hover:scale-[1.02] active:scale-95 transition-all">
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-10">
          <div class="bg-slate-50 p-6 rounded-2xl flex flex-col border border-slate-100">
            <span class="text-[10px] font-bold uppercase tracking-widest text-on-surface-variant mb-2">Active Galleries</span>
            <span class="text-3xl font-black text-primary">{{ stats.total }}</span>
          </div>
          <div class="bg-slate-50 p-6 rounded-2xl flex flex-col border border-slate-100">
            <span class="text-[10px] font-bold uppercase tracking-widest text-on-surface-variant mb-2">Stored Photos</span>
            <span class="text-3xl font-black text-primary">{{ stats.totalMediaDisplay }}</span>
          </div>
        ...
        <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          ...
          <div 
            ...
            :key="school.id"
            class="group bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 border border-slate-50 flex flex-col"
          >
            ...
            <div class="p-6 flex-1 flex flex-col">
              <div class="flex items-start justify-between mb-4">
                <div>
                  <h3 class="text-lg font-bold text-on-surface leading-snug group-hover:text-primary transition-colors">
                    {{ school.name }}
                  </h3>
                  <div class="flex items-center gap-2 mt-1">
                    <span class="material-symbols-outlined text-sm text-on-surface-variant">photo_library</span>
                    <span class="text-sm font-medium text-on-surface-variant">{{ (school.gallery_count || 0).toLocaleString() }} Photos</span>
                  </div>
                </div>
              ...
              <div class="mt-auto pt-4 flex gap-2 border-t border-slate-50">
                <button 
                  @click="goToGallery(school.id)"
                  class="flex-1 bg-slate-50 text-slate-600 font-bold text-xs py-3 rounded-lg hover:bg-primary hover:text-white transition-all"
                >
                  Open Archive
                </button>
                <button class="w-10 h-10 flex items-center justify-center bg-slate-50 text-slate-400 rounded-lg hover:bg-slate-100 transition-colors">
          ...
          <button @click="openAddModal" class="group relative bg-white border-2 border-dashed border-slate-100 rounded-2xl flex flex-col items-center justify-center p-8 hover:border-primary/20 hover:bg-slate-50/30 transition-all duration-300 min-h-[400px]">
            <div class="w-16 h-16 rounded-full bg-slate-50 flex items-center justify-center mb-4 group-hover:bg-primary/5 transition-colors">
              <span class="material-symbols-outlined text-3xl text-slate-300 group-hover:text-primary">add_photo_alternate</span>
            </div>
            <h3 class="text-lg font-bold text-slate-900">Initialize New Gallery</h3>
            <p class="text-sm text-slate-400 text-center mt-2 max-w-[200px]">Create an archive for another partner institution.</p>
          </button>
        </div>
      </div>
    </main>
