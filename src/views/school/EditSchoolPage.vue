  ...
  try {
    ...
    if (data.success) {
      ...
      school.value = {
        ...
        principal_designation: s.principal_designation || 'Principal',
        email: s.email || '',
        phone: s.phone || '',
        show_principal_signature: s.show_principal_signature == 1,
        head_coach_name: s.head_coach_name || '',
        head_coach_designation: s.head_coach_designation || 'Head Coach',
        show_head_coach_signature: s.show_head_coach_signature == 1
      };
  ...
  <div class="px-0 py-0 school-page-wrapper">
    ...
    <template v-else>
      ...
      <nav class="flex items-center gap-2 mb-6 text-xs font-medium text-on-surface-variant px-4">
        ...
        <span class="text-on-surface">Edit School</span>
      </nav>

      <!-- Page Title -->
      <div class="mb-10 flex flex-col sm:flex-row sm:items-end justify-between gap-4 px-4">
        <div>
          <h1 class="text-3xl font-extrabold text-on-surface tracking-tight leading-none mb-2">Edit School</h1>
          <p class="text-on-surface-variant text-sm font-medium">Update institutional details and operational contacts.</p>
        </div>
      ...
      <div class="space-y-10 pb-12">
        ...
        <section class="bg-surface-container-lowest rounded-xl p-8 shadow-[0_8px_32_rgba(0,28,58,0.04)] mx-4">
